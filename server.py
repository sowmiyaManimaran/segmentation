...
# Now you can use TensorFlow 1.x constructs, but this is not recommended for new code
...
def create_demographics(df):
    demographics = df.groupby(['City', 'State', 'Country', 'Brand']).size().reset_index(name='Count')
    
    # Assign the recommended brand based on count
    demographics['Brand_Recommend'] = 'Recommend Popular'
    demographics.loc[demographics['Count'] >= 2, 'Brand_Recommend'] = demographics['Brand']
    
    # Group by City and State, and aggregate recommended brands
    demographics = demographics.groupby(['City', 'State'])['Brand_Recommend'].apply(list).reset_index()
    
    # Convert the list of recommended brands to a comma-separated string
    demographics['Brand_Recommend'] = demographics['Brand_Recommend'].apply(lambda x: ', '.join(map(str, x)))
...
# ct_x1 = ColumnTransformer([('my_ohe', OneHotEncoder(), [0, 1])], remainder='passthrough')
# predict_model_for_city_X = ct_x1.fit_transform(predict_model_for_city_X)
...
# ct = ColumnTransformer([('my_ohe', OneHotEncoder(), [0])], remainder='passthrough')
# predict_score_X = ct.fit_transform(predict_score_X)
predict_score_X = predict_score_X[:,1:]
sc_predict_score_X = StandardScaler(with_mean=False)
sc_predict_score_y = StandardScaler(with_mean=False)
